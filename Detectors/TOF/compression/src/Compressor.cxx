...
#ifdef CHECKER_VERBOSE
      if (mCheckerVerbose) {
        printf(" TRM EventCounter / DRM LocalEventCounter mismatch: %d / %d (slotId=%u) \n", eventCnt, locEvCnt, slotId);
      }
#endif
    }
...
#ifdef CHECKER_VERBOSE
        if (mCheckerVerbose) {
          printf(" TRM Chain EventCounter / DRM LocalEventCounter mismatch: %d / %d (slotId=%u, chain=%d) \n", eventCnt, locEvCnt, slotId, ichain);
        }
#endif
      }
...
#ifdef CHECKER_VERBOSE
        if (mCheckerVerbose) {
          printf(" TRM Chain bad Status: %d (slotId=%u, chain=%d) \n", status, slotId, ichain);
        }
#endif
      }
...
#ifdef CHECKER_VERBOSE
        if (mCheckerVerbose) {
          printf(" TRM Chain BunchID / DRM L0BCID mismatch: %d / %d (slotId=%u, chain=%d) \n", bunchCnt, gbtBunchCnt, slotId, ichain);
        }
#endif
      }
