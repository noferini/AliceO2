...
Double_t* AliTOFT0v2::DefineT0(Option_t* option, Float_t pMinCut, Float_t pMaxCut, Int_t isCalibrationMode)
{
  ...
  for (Int_t itrk = 0; itrk < ntrk; itrk++) {
    ...
    if (!(mom <= fUpperMomBound && mom >= fLowerMomBound))
      continue;

    if (!AcceptTrack(t))
      continue;

    if (t->GetIntegratedLength() < 350)
      continue; //skip decays
    if (t->GetP() > pMinCut && t->GetP() < pMaxCut)
      continue;

    if (isCalibrationMode && sample != isCalibrationMode)
      continue;
    ...
    if (fStartTimeIndex[ntrackmapped - 1] != -1)
      continue;
  ...
  if (nmaxtracksinsetCurrent * nseteq < ngoodtrkt0)
  ...
  if (ngoodtrkt0 >= 2) {
    ...
    if (ngoodtrkt0 > nmaxtracksinsetCurrent) {
      nset = (Int_t)(ngoodtrkt0 / ntracksinset) + 1;
    }
    ...
    if (nset >= 1) {
      for (Int_t i = 0; i < nset; i++) {
        ...
        for (Int_t itz = 0; itz < ntracksinsetmy; itz++) {
          beta[itz] = momentum[itz] / sqrt(massarray[0] * massarray[0] + momentum[itz] * momentum[itz]);
        ...
        if (fOptFlag)
          ncombinatorial = fLookupPowerThree[ntracksinsetmy];
        else
          ncombinatorial = ToCalculatePower(3, ntracksinsetmy);
        ...
        for (Int_t k = 0; k < ncombinatorial; k++) {
          for (Int_t j = 0; j < ntracksinsetmy; j++) {
            ...
            if (fOptFlag)
              dtexp[j] = momErr[j][imass[j]]; // see comments above in the initialisation of momErr
            else
        ...
        for (Int_t icsq = 0; icsq < ntracksinsetmy; icsq++) {
          ...
          if (TMath::Prob(bestchisquare[icsq], 1) < confLevelThr) {
            ...
            if (internalMapping[index] > -1)
              fStartTimeIndex[internalMapping[index]] = -1;
  ...
  if (fT0SigmaT0def[1] < 0.00001)
    fT0SigmaT0def[1] = 0.6;

  Int_t nused = 0;
  for (Int_t i = 0; i < 2000; i++) {
    if (fStartTimeIndex[i] == fCurrentStartTime)
      nused++;
  }
