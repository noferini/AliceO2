...
void TOFMatcherSpec::run(ProcessingContext& pc)
{
  ...
  if (isITSused) {
    pc.outputs().snapshot(Output{o2::header::gDataOriginTOF, "MTC_ITS", 0}, mMatcher.getMatchedTrackVector(o2::dataformats::MatchInfoTOFReco::TrackType::ITS));
    if (mUseMC) {
      pc.outputs().snapshot(Output{o2::header::gDataOriginTOF, "MCMTC_ITS", 0}, mMatcher.getMatchedTOFLabelsVector(o2::dataformats::MatchInfoTOFReco::TrackType::ITS));
    }
  }
  if (isTPCTRDused) {
