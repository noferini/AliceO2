...
class MatchTOF
{
 ...
 protected:
  ...
  void doMatching(int sec, trkType type = trkType::ITSTPC);
  ...
  gsl::span<const o2::MCCompLabel> mTPCLabels[trkType::SIZE]; ///< TPC label of input tracks (vector to read from tree)
  ...
  std::vector<o2::dataformats::MatchInfoTOF> mMatchedTracks[trkType::SIZE]; // this is the output of the matching
  std::vector<o2::MCCompLabel> mOutTOFLabels[trkType::SIZE];                ///< TOF label of matched tracks

  int mNumOfTracks[trkType::SIZE];                     // number of tracks to be matched
